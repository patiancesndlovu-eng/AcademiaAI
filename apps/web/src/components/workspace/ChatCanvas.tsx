...
import { ArrowRight, SlidersHorizontal, MoreVertical, BrainCircuit, Sparkles } from "lucide-react";
...
export function ChatCanvas({ notebookTitle, sourceCount, onToast }: ChatCanvasProps) {
    ...
    <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-[#202226]">
      <div className="flex items-center justify-between border-b border-[#30343b] px-5 py-3.5">
        ...
        <div className="flex items-center gap-1">
          <IconButton label="Chat settings" onClick={() => onToast("Chat settings opened")}><SlidersHorizontal size={17} /></IconButton>
          <IconButton label="Chat options" onClick={() => onToast("Chat options opened")}><MoreVertical size={18} /></IconButton>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-6 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-[720px] pt-9">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onToast("Notebook response style set to concise") } className="flex items-center gap-2 rounded-full border border-[#3c414b] px-3.5 py-2 text-xs font-medium text-[#d4d8e1] transition hover:border-[#65708b]">
              <Sparkles size={14} className="text-[#9cb9ff]" /> Customize
            </button>
          </div>
          <h1 className="mt-8 max-w-[710px] font-display text-[29px] leading-[1.12] tracking-[-0.048em] text-[#f0f2f6] sm:text-[34px]">{notebookTitle}</h1>
          <p className="mt-3 text-[13px] text-[#a2a9b5]">{sourceCount} sources · Updated 27 Aug 2026</p>
      ...
      <div className="border-t border-[#30343b] px-5 py-4 sm:px-8">
        <div className="mx-auto flex max-w-[720px] items-end gap-3 rounded-[17px] border border-[#4a4f59] bg-[#24272c] p-3 shadow-[0_10px_30px_rgba(0,0,0,.12)] transition focus-within:border-[#748bc5] focus-within:ring-1 focus-within:ring-[#5f75b1]">
          <textarea
            value={prompt}
          ...
          <button onClick={sendPrompt} aria-label="Send message" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#6f8fef] text-[#141a2b] transition hover:bg-[#91aaff] active:scale-95 disabled:cursor-not-allowed disabled:opacity-40" disabled={!prompt.trim()}>
            <ArrowRight size={18} />
          </button>
        </div>
