...
import { PanelLeft, Sparkles, Clipboard, ChevronRight, Headphones, Layers3, Video, Network, FileCheck2, BookOpenCheck, HelpCircle, ImageIcon, Grid2X2 } from "lucide-react";

type StudioKind = "audio" | "slides" | "video" | "map" | "report" | "flashcards" | "quiz" | "infographic" | "table";

const studioItems: { kind: StudioKind; title: string; description: string; tint: string; icon: ReactNode; beta?: boolean }[] = [
  ...
  return (
    <aside className="flex min-h-0 w-full flex-col border-t border-[#30343b] bg-[#1e2024] xl:w-[318px] xl:border-l xl:border-t-0">
      <div className="flex items-center justify-between border-b border-[#30343b] px-4 py-3.5">
        ...
        <button onClick={() => onToast("Studio panel expanded")} className="text-[#aeb4bf] hover:text-white"><PanelLeft size={17} /></button>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {studioItems.map((item) => (
            <button key={item.kind} onClick={() => item.kind === "quiz" ? onQuiz() : onToast(`${item.title} is ready to configure`)} className={`group min-h-[74px] rounded-xl border border-white/[0.04] ${item.tint} p-3 text-left transition hover:-translate-y-0.5 hover:border-white/20 hover:brightness-110`}>
